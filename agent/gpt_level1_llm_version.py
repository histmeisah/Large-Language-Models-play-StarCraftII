...
import os
import numpy as np
from summarize.level_one.gpt_level1 import L1_LLM_version
from summarize.gpt_test.L2_summarize import L2_summary
import wandb
from collections import deque
from utils.action_extractor_v2 import *
from utils.action_vector_test import ActionDBManager
...
class ChatGPTAgent:
    ...
    def __init__(self, model_name, api_key, api_base, system_prompt, example_prompt, temperature, args,
        ...
        self.L1 = L1_LLM_version(api_key=self.api_key, api_base=self.api_base, temperture=self.temperature)
        self.get_opposite_bot_name()
        self.base_save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../starcraftenv_test', 'log', 'chatgpt_log')
    ...
    def _wandb_save_log(self, raw_observation, L1_observation, commander, action):
        """
        保存观测信息
        :param raw_observation:
        :param L1_observation:
        :param commander:
        :param action:
        :return:
        """
        raw_observation_str = json.dumps(raw_observation, indent=2)
        L1_observation_str = json.dumps(L1_observation, indent=2)
        commander_str = json.dumps(commander, indent=2)

        # Convert action from int to str using action_dict
        flat_dict = {}
        for key, value in self.action_dict.items():
            for inner_key, inner_value in value.items():
                flat_dict[inner_key] = inner_value
        action_str = flat_dict[action]

        # Log to Weights & Biases
        wandb.log({
            "raw_observation": raw_observation_str,
            "L1_observation": L1_observation_str,
    ...
    def action(self, observation):
        ...
        if isinstance(raw_observation, dict):
            string_observation = json.dumps(raw_observation)# Convert the observation to a string

            L1_observation = self.L1.query(string_observation)  # Generate a level 1 summary
        ...
        self._wandb_save_log(raw_observation=raw_observation, L1_observation=L1_observation,
                             commander=self.temp_command, action=action)
