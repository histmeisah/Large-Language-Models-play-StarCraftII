...
class RandomAgent:
    ...
    def __init__(self, model_name, api_key, api_base, system_prompt, example_prompt, temperature, args,
        ...
        # 定义基础保存目录
        self.base_save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../starcraftenv_test', 'log', 'random_log')

        # 为每一局游戏创建一个独立的文件夹，这里使用当前时间作为唯一标识符
        # 注意：您需要定义self.current_time 和 self.process_id
        self.game_folder = os.path.join(self.base_save_dir, f"game_{self.current_time}_process_id_{self.process_id}")
