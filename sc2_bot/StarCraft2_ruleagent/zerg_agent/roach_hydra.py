...
class roach_hydra_bot(BotAI):
    ...
    async def procedure(self):
        ...
        if self.time_formatted < "01:00":
            ...
            if self.structures(UnitTypeId.EVOLUTIONCHAMBER).amount==2 and len(self.upgrade_tag_list)<2:
                for bv in self.structures(UnitTypeId.EVOLUTIONCHAMBER):
                    if bv not in self.upgrade_tag_list:
                        self.upgrade_tag_list.append(bv)

            if self.structures(UnitTypeId.HYDRALISKDEN).amount + self.already_pending(UnitTypeId.HYDRALISKDEN) == 0:
                if self.can_afford(UnitTypeId.HYDRALISKDEN) and self.workers.amount >= 60:
                    if self.structures(UnitTypeId.LAIR).ready and self.structures(UnitTypeId.SPAWNINGPOOL).ready:
                        ...
                        placement_position = await self.find_placement(UnitTypeId.HYDRALISKDEN, near=place_postion,
                                                                       placement_step=6)
                        if placement_position and self.already_pending(UnitTypeId.HYDRALISKDEN) == 0:
                            build_worker = worker_candidates.closest_to(placement_position)
                            build_worker.build(UnitTypeId.HYDRALISKDEN, placement_position)

            bvs = self.structures(UnitTypeId.EVOLUTIONCHAMBER)
            if len(self.upgrade_tag_list)==2 :
                if self.upgrade_tag_list[0].is_idle :
                    if self.already_pending_upgrade(UpgradeId.ZERGMISSILEWEAPONSLEVEL1) == 0 and self.can_afford(
                            UpgradeId.ZERGMISSILEWEAPONSLEVEL1):
                        self.upgrade_tag_list[0].research(UpgradeId.ZERGMISSILEWEAPONSLEVEL1)
                if self.upgrade_tag_list[1].is_idle:
                    if self.already_pending_upgrade(UpgradeId.ZERGGROUNDARMORSLEVEL1) == 0 and self.can_afford(
                            UpgradeId.ZERGGROUNDARMORSLEVEL1):
                        self.upgrade_tag_list[1].research(UpgradeId.ZERGGROUNDARMORSLEVEL1)
            ...
            bvs = self.structures(UnitTypeId.EVOLUTIONCHAMBER)
            for bv in bvs.ready.idle:
                if self.already_pending_upgrade(UpgradeId.ZERGMISSILEWEAPONSLEVEL3) == 0 and self.can_afford(
    ...
    def hydra_upgrades(self):
        hydraden = self.structures(UnitTypeId.HYDRALISKDEN)
        if hydraden:
            if hydraden and self.workers.amount>=60:
                for den in hydraden:
                    if den.is_idle  and self.can_afford(UpgradeId.EVOLVEGROOVEDSPINES):
                        den.research(UpgradeId.EVOLVEGROOVEDSPINES)
                    if den.is_idle and self.can_afford(UpgradeId.EVOLVEMUSCULARAUGMENTS):
    ...
    async def build_vespene(self):
        ...
        for i in range(4):
            base_ = base[i]
            gases = self.vespene_geyser.closer_than(10, base_)
            for gas in gases:
    ...
    async def roach_bulid(self):
        ...
        if 60 <= self.workers.amount + self.already_pending(UnitTypeId.DRONE) <= 70:
            ...
            if self.can_afford(UnitTypeId.ROACH) and self.structures(UnitTypeId.ROACHWARREN).ready:
                if larvae and self.can_afford(UnitTypeId.ROACH) and self.supply_left >= 4 and self.units(
                        UnitTypeId.ROACH).amount + self.already_pending(UnitTypeId.ROACH) < 30:
                    larvae.random.train(UnitTypeId.ROACH)
        else:
            pass
    ...
    async def hydra_bulid(self):
        ...
        if self.can_afford(UnitTypeId.HYDRALISK) and self.structures(UnitTypeId.HYDRALISKDEN).ready:
            if larvae and self.can_afford(UnitTypeId.HYDRALISK) and self.supply_left >= 4 and self.already_pending(
                    UnitTypeId.HYDRALISK) + self.units(UnitTypeId.HYDRALISK).amount < 25:
    ...
    async def queen_build(self):
        for base in self.townhalls:
            if base.is_idle and self.can_afford(UnitTypeId.QUEEN) and self.units(UnitTypeId.QUEEN).amount < 6:
    ...
    async def attack(self):
        ...
        if self.supply_army > 90 and self.rally_defend == False:
            target = self.select_target()
            for unit in self.units.of_type(
                    {UnitTypeId.ROACH, UnitTypeId.ZERGLING, UnitTypeId.OVERSEER, UnitTypeId.ULTRALISK,UnitTypeId.BANELING,UnitTypeId.HYDRALISK}):
...
def main():
    run_game(
        ...
        [Bot(Race.Zerg, roach_hydra_bot()), Computer(Race.Terran, Difficulty.Hard)],
        realtime=False,
