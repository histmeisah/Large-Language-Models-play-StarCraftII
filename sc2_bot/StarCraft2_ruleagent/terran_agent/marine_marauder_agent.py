...
class marine_marauder_Bot(BotAI):
    # (55.5, 157.5)左上
    # (160.5, 46.5)右下
    def __init__(self):
    ...
    async def procedure(self):
        ...
        if self.time_formatted == '00:43' and not self.already_pending(UnitTypeId.BARRACKS):
            ...
            placement_position = await self.find_placement(UnitTypeId.BARRACKS, near=place_postion.towards(self.game_info.map_center, 6), placement_step=5)
            ...
            placement_position = await self.find_placement(UnitTypeId.BARRACKS, near=place_postion, placement_step=5)
            if placement_position:
                build_worker = worker_candidates.closest_to(placement_position)
            ...
            if self.structures(UnitTypeId.ORBITALCOMMAND).find_by_tag(self.center_tag_list[0]).position and self.already_pending(UnitTypeId.ENGINEERINGBAY)+self.units(UnitTypeId.ENGINEERINGBAY).amount<2\
                ...
                location2 = self.structures(UnitTypeId.ORBITALCOMMAND).find_by_tag(
                    self.center_tag_list[0]).position + Point2((self.Location * 8, -self.Location * 1))
        ...
        if self.minerals>1000:
            if not self.already_pending(UnitTypeId.COMMANDCENTER) and self.can_afford(UnitTypeId.COMMANDCENTER):
                ...
                if location:
                    ...
                    if worker and self.can_afford(UnitTypeId.COMMANDCENTER):
                        worker.build(UnitTypeId.COMMANDCENTER, location)
        if self.minerals > 1000 and self.time_formatted <= '07:00':
            if not self.already_pending(UnitTypeId.COMMANDCENTER) and self.can_afford(UnitTypeId.COMMANDCENTER):
                location = await self.get_next_expansion()
                if location:
                    worker = self.select_build_worker(location)
                    if worker and self.can_afford(UnitTypeId.COMMANDCENTER):
                        worker.build(UnitTypeId.COMMANDCENTER, location)
            if self.structures(UnitTypeId.BARRACKS).amount < 4 and not self.already_pending(
                ...
                if self.can_afford(UnitTypeId.BARRACKS) and not self.already_pending(UnitTypeId.BARRACKS):
                    ...
                    if placement_position:
                        build_worker = worker_candidates.closest_to(placement_position)
                        build_worker.build(UnitTypeId.BARRACKS, placement_position)
        if self.minerals > 1000 and '07:00' <= self.time_formatted <= '09:00':
            if self.structures(UnitTypeId.BARRACKS).amount < 8 and not self.already_pending(
                    UnitTypeId.BARRACKS) and self.can_afford(UnitTypeId.BARRACKS):
                if self.can_afford(UnitTypeId.BARRACKS) and not self.already_pending(UnitTypeId.BARRACKS):
                    building_place = self.third_base.position
                    # self.build(UnitTypeId.PYLON, near=nexus.position.towards(self.game_info.map_center, 5))
                    ...
                    if placement_position:
                        build_worker = worker_candidates.closest_to(placement_position)
                        build_worker.build(UnitTypeId.BARRACKS, placement_position)
            if self.structures(UnitTypeId.FACTORY).amount < 2 and not self.already_pending(
                ...
                if self.can_afford(UnitTypeId.FACTORY) and not self.already_pending(UnitTypeId.FACTORY):
                    building_place = self.main_base.position + Point2((0, -self.Location * 3))
                    placement_position = await self.find_placement(UnitTypeId.BARRACKS, near=building_place,
                                                                   placement_step=2)
                    worker_candidates = self.workers.filter(lambda worker: (
                                                                                   worker.is_collecting or worker.is_idle) and worker.tag not in self.unit_tags_received_action)
            ...
            if self.structures(UnitTypeId.STARPORT).amount < 2 and not self.already_pending(
                ...
                if self.can_afford(UnitTypeId.STARPORT) and not self.already_pending(UnitTypeId.STARPORT):
                    ...
                    if placement_position:
                        build_worker = worker_candidates.closest_to(placement_position)
                        build_worker.build(UnitTypeId.STARPORT, placement_position)
        if self.minerals > 1000 and '09:00' <= self.time_formatted:
            if self.structures(UnitTypeId.BARRACKS).amount < 12 and not self.already_pending(
                    UnitTypeId.BARRACKS) and self.can_afford(UnitTypeId.BARRACKS):
                if self.can_afford(UnitTypeId.BARRACKS) and not self.already_pending(UnitTypeId.BARRACKS):
                    ...
                    if placement_position:
                        build_worker = worker_candidates.closest_to(placement_position)
                        build_worker.build(UnitTypeId.BARRACKS, placement_position)
            if self.structures(UnitTypeId.FACTORY).amount < 3 and not self.already_pending(
                    UnitTypeId.FACTORY) and self.can_afford(UnitTypeId.FACTORY):
                if self.can_afford(UnitTypeId.FACTORY) and not self.already_pending(UnitTypeId.FACTORY):
                    building_place = self.fourthbase.position + Point2((0, -self.Location * 3))
                    placement_position = await self.find_placement(UnitTypeId.BARRACKS, near=building_place,
    ...
    async def attack(self):
        ...
        if self.supply_army > 100 and self.rally_defend == False:
            ...
            for unit in self.units.of_type({UnitTypeId.MARINE, UnitTypeId.SIEGETANK, UnitTypeId.MARAUDER}):
...
def main():
    run_game(  # run_game is a function that runs the game.
        ...
        [Bot(Race.Terran, marine_marauder_Bot()),
