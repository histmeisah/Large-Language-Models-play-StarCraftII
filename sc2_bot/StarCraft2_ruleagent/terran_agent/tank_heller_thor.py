...
class tank_heller_thor_Bot(BotAI):
    # (55.5, 157.5)左上
    # (160.5, 46.5)右下
    def __init__(self):
    ...
    async def on_step(self, iteration: int):
        ...
        if self.producing_marauder == True:
            await self.train_marauder()
        if self.producing_thor==True:
            await self.train_thor()
        if self.units(UnitTypeId.ORBITALCOMMAND).amount>=3:
            self.build_planetary=True
        if self.build_planetary==True:
            await self.upgrade_planetary()
    async def produce_worker(self):
        ...
        if self.townhalls and self.supply_workers <= 66:
    ...
    async def procedure(self):
        ...
        if self.time_formatted == '00:43' and not self.already_pending(UnitTypeId.BARRACKS):
            ...
            place_postion = self.start_location.position
            placement_position = await self.find_placement(UnitTypeId.BARRACKS,
                                                           near=place_postion.towards(self.game_info.map_center, 6),
                                                           placement_step=5)
            ...
            placement_position = await self.find_placement(UnitTypeId.BARRACKS, near=place_postion, placement_step=3)
            if placement_position:
                build_worker = worker_candidates.closest_to(placement_position)
            ...
            placement_position = await self.find_placement(UnitTypeId.BARRACKS, near=place_postion.towards(self.game_info.map_center), placement_step=3)
            if placement_position:
                build_worker = worker_candidates.closest_to(placement_position)
                build_worker.build(UnitTypeId.FACTORY, placement_position)
        ...
        elif self.time_formatted == '06:26':
            if self.can_afford(UnitTypeId.STARPORT) and not self.already_pending(UnitTypeId.STARPORT):
                building_place = self.start_location
                placement_position = await self.find_placement(UnitTypeId.STARPORT, near=building_place.towards(self.game_info.map_center,5),
                                                               placement_step=2)
                worker_candidates = self.workers.filter(lambda worker: (
                                                                               worker.is_collecting or worker.is_idle) and worker.tag not in self.unit_tags_received_action)
                if placement_position:
        ...
        elif '04:40' <= self.time_formatted <= '04:50':
            if self.structures(UnitTypeId.ORBITALCOMMAND).find_by_tag(
                    self.center_tag_list[0]).position and self.already_pending(UnitTypeId.ENGINEERINGBAY) + self.units(
                    UnitTypeId.ENGINEERINGBAY).amount < 1 \
                    or self.units(UnitTypeId.BUNKER).amount + self.already_pending(UnitTypeId.BUNKER) < 1:
                ...
                location2 = self.second_base.position + Point2((self.Location * 8, -self.Location * 1))
                placement_position = await self.find_placement(UnitTypeId.ENGINEERINGBAY, near=location2,
                                                               placement_step=1)
                if self.can_afford(UnitTypeId.ENGINEERINGBAY) and self.already_pending(UnitTypeId.ENGINEERINGBAY) < 1:
        ...
        elif self.time_formatted == '07:10':
            ...
            worker_candidates = self.workers.filter(lambda worker: (
                                                                           worker.is_collecting or worker.is_idle) and worker.tag not in self.unit_tags_received_action)
            place_postion = self.main_base.position
            placement_position = await self.find_placement(UnitTypeId.BARRACKS, near=place_postion.towards(self.game_info.map_center), placement_step=3)
            if placement_position:
                build_worker = worker_candidates.closest_to(placement_position)
                build_worker.build(UnitTypeId.FACTORY, placement_position)
            if self.units(UnitTypeId.FACTORYTECHLAB).exists and not self.already_pending(UpgradeId.HIGHCAPACITYBARRELS):
                self.research(UpgradeId.HIGHCAPACITYBARRELS)
            ...
            self.producing_thor=True
            self.trigger_extra_supply = True
            for base in self.townhalls:
                if base != self.main_base and base != self.second_base:
                    self.third_base = base
            ...
            if self.can_afford(UnitTypeId.ARMORY) and self.already_pending(UnitTypeId.ARMORY) < 2:
                building_place = self.third_base.position + Point2((-self.Location * 12, -self.Location * 0))
                placement_position = await self.find_placement(UnitTypeId.ARMORY, near=building_place,
                                                               placement_step=2)
                worker_candidates = self.workers.filter(lambda worker: (
                                                                               worker.is_collecting or worker.is_idle) and worker.tag not in self.unit_tags_received_action)
            ...
            if self.can_afford(UpgradeId.TERRANVEHICLEWEAPONSLEVEL1):
                self.research(UpgradeId.TERRANVEHICLEWEAPONSLEVEL1)
            if self.can_afford(UpgradeId.TERRANVEHICLEANDSHIPARMORSLEVEL1):
                self.research(UpgradeId.TERRANVEHICLEANDSHIPARMORSLEVEL1)
            if self.can_afford(UpgradeId.TERRANINFANTRYWEAPONSLEVEL1):
                self.research(UpgradeId.TERRANINFANTRYWEAPONSLEVEL1)
        elif '07:30' <= self.time_formatted <= '08:30':
            for base in self.townhalls:
                if base != self.main_base and base != self.second_base and base != self.third_base:
                    self.fourthbase = base
            ...
            #     print(self.structures.find_by_tag(self.building_tag_list[1]).is_ready)
            #     self.structures.find_by_tag(self.building_tag_list[1])(AbilityId.LIFT_FACTORY)
        elif self.time_formatted>'08:30':
            if self.structures(UnitTypeId.ARMORY).idle and self.can_afford(UpgradeId.TERRANVEHICLEWEAPONSLEVEL2) and self.already_pending(UpgradeId.TERRANVEHICLEWEAPONSLEVEL2)==0:
                self.research(UpgradeId.TERRANVEHICLEWEAPONSLEVEL2)
            # if self.structures(UnitTypeId.ARMORY).idle and self.can_afford(UpgradeId.TERRANVEHICLEARMORSLEVEL2) and self.already_pending(UpgradeId.TERRANVEHICLEARMORSLEVEL2)==0:
            #     self.research(UpgradeId.TERRANVEHICLEARMORSLEVEL2)
        ...
        if self.time_formatted<='06:00':
            if self.supply_left <= 4 and not self.already_pending(UnitTypeId.SUPPLYDEPOT) and not self.supply_cap == 200:
                ...
                if self.trigger_extra_supply == True:
                    place_postion2 = self.structures.find_by_tag(self.center_tag_list[0]).position + Point2(
                        (0, self.Location * 8))
                    await self.build(UnitTypeId.SUPPLYDEPOT, near=place_postion2, placement_step=2)
        elif '06:00'<=self.time_formatted<='09:00':
            if self.supply_left <= 6 and  self.already_pending(
                    UnitTypeId.SUPPLYDEPOT)<2 and not self.supply_cap == 200:
                if self.can_afford(UnitTypeId.SUPPLYDEPOT):
                    place_postion = self.second_base.position + Point2((0, self.Location * 8))
                    await self.build(UnitTypeId.SUPPLYDEPOT, near=place_postion, placement_step=2)
        elif '09:00'<=self.time_formatted<='11:00':
            if self.supply_left <= 6 and  self.already_pending(
                    UnitTypeId.SUPPLYDEPOT)<3 and not self.supply_cap == 200:
                if self.can_afford(UnitTypeId.SUPPLYDEPOT):
                    place_postion = self.third_base.position + Point2((0, self.Location * 8))
                    await self.build(UnitTypeId.SUPPLYDEPOT, near=place_postion, placement_step=2)
    ...
    async def upgrade_center(self):
        if self.structures(UnitTypeId.COMMANDCENTER).exists and self.can_afford(UnitTypeId.ORBITALCOMMAND) and self.townhalls.amount<=3:
        ...
        if self.structures(UnitTypeId.ORBITALCOMMAND).ready:
            self.flag = True
    async def upgrade_planetary(self):
        if self.structures(UnitTypeId.COMMANDCENTER).exists and self.can_afford(UnitTypeId.PLANETARYFORTRESS) and self.structures(UnitTypeId.ENGINEERINGBAY).exists:
            center = self.structures(UnitTypeId.COMMANDCENTER).random
            center(AbilityId.UPGRADETOPLANETARYFORTRESS_PLANETARYFORTRESS)

    async def expansion(self):
        if self.time_formatted<='09:00':
            if self.minerals > 1000:
                if not self.already_pending(UnitTypeId.COMMANDCENTER) and self.can_afford(UnitTypeId.COMMANDCENTER) and self.townhalls.amount+self.already_pending(UnitTypeId.COMMANDCENTER)<4:
                    location = await self.get_next_expansion()
                    if location:
                        worker = self.select_build_worker(location)
                        if worker and self.can_afford(UnitTypeId.COMMANDCENTER):
            ...
            if self.minerals > 1000:
                if not self.already_pending(UnitTypeId.COMMANDCENTER) and self.can_afford(UnitTypeId.COMMANDCENTER) and self.townhalls.amount+self.already_pending(UnitTypeId.COMMANDCENTER)<6:
                    location = await self.get_next_expansion()
                    if location:
                        worker = self.select_build_worker(location)
                        if worker and self.can_afford(UnitTypeId.COMMANDCENTER):
        ...
        if self.minerals > 500 and self.vespene >300 and self.time_formatted <= '08:00':
            if self.structures(UnitTypeId.FACTORY).amount < 4 and not self.already_pending(
                ...
                if self.can_afford(UnitTypeId.FACTORY) and not self.already_pending(UnitTypeId.FACTORY):
                    ...
                    placement_position = await self.find_placement(UnitTypeId.FACTORY, near=building_place.towards(self.game_info.map_center,5),
                                                                   placement_step=2)
                    worker_candidates = self.workers.filter(lambda worker: (
                                                                                   worker.is_collecting or worker.is_idle) and worker.tag not in self.unit_tags_received_action)
                    if placement_position:
                        build_worker = worker_candidates.closest_to(placement_position)
                        build_worker.build(UnitTypeId.FACTORY, placement_position)
        if self.minerals > 700 and self.vespene>400 and '08:00' <= self.time_formatted <= '10:00':
            if self.structures(UnitTypeId.FACTORY).amount < 6 and not self.already_pending(
                    UnitTypeId.FACTORY) and self.can_afford(UnitTypeId.FACTORY):
                if self.can_afford(UnitTypeId.FACTORY) and not self.already_pending(UnitTypeId.FACTORY):
                    building_place = self.third_base.position + Point2((0, -self.Location * 3))
                    placement_position = await self.find_placement(UnitTypeId.BARRACKS, near=building_place.towards(self.game_info.map_center,5),
                                                                   placement_step=2)
                    worker_candidates = self.workers.filter(lambda worker: (
                                                                                   worker.is_collecting or worker.is_idle) and worker.tag not in self.unit_tags_received_action)
                    if placement_position:
            ...
            if self.structures(UnitTypeId.STARPORT).amount < 2 and not self.already_pending(
                    UnitTypeId.STARPORT) and self.can_afford(UnitTypeId.STARPORT):
                if self.can_afford(UnitTypeId.STARPORT) and not self.already_pending(UnitTypeId.STARPORT):
                    building_place = self.second_base.position + Point2((0, -self.Location * 3))
                    ...
                    if placement_position:
                        build_worker = worker_candidates.closest_to(placement_position)
                        build_worker.build(UnitTypeId.STARPORT, placement_position)
        if self.minerals > 1000 and '12:00' <= self.time_formatted and self.townhalls.amount>=4:

            if self.structures(UnitTypeId.FACTORY).amount < 8 and not self.already_pending(
                    UnitTypeId.FACTORY) and self.can_afford(UnitTypeId.FACTORY):
                if self.can_afford(UnitTypeId.FACTORY) and not self.already_pending(UnitTypeId.FACTORY):
                    building_place = self.fourthbase.position + Point2((0, -self.Location * 3))
                    placement_position = await self.find_placement(UnitTypeId.BARRACKS, near=building_place,
                                                                   placement_step=2)
                    worker_candidates = self.workers.filter(lambda worker: (
                                                                                   worker.is_collecting or worker.is_idle) and worker.tag not in self.unit_tags_received_action)
    ...
    async def add_on_after_exp(self):
        if self.time_formatted<='07:00':
            ...
            if self.units(UnitTypeId.FACTORYTECHLAB).amount < 5:
            ...
            if self.units(UnitTypeId.FACTORYTECHLAB).amount >= 5:
                for fac in self.structures(UnitTypeId.FACTORY):
                    if not fac.has_add_on:
                        fac(AbilityId.BUILD_REACTOR_FACTORY)
            for star in self.structures(UnitTypeId.STARPORT):
                if not star.has_add_on:
                    star(AbilityId.BUILD_REACTOR_STARPORT)
                    self.starport_tag_list.append(star)
        elif self.time_formatted>='07:00':
            if self.units(UnitTypeId.FACTORYTECHLAB).amount <= 7:
                for fac in self.structures(UnitTypeId.FACTORY):
                    if not fac.has_add_on:
                        fac(AbilityId.BUILD_TECHLAB_FACTORY)
                        self.factory_tag_list.append(fac)
    ...
    async def marauder_movement(self):
        if self.units(UnitTypeId.MARAUDER).exists:
            enemies = self.enemy_units | self.enemy_structures
            for r in self.units(UnitTypeId.MARAUDER):
                enemy_threats_close: Units = enemies.filter(
                    lambda unit: unit.distance_to(r) < 20
                )
                ...
                if r.health_percentage > 2 / 5 and enemy_units and not r.has_buff(
                        BuffId.STIMPACK) and enemy_units.amount >= 10:
                    r(AbilityId.EFFECT_STIM_MARAUDER)
    ...
    async def train_tank(self):
        if self.time_formatted<='06:00':
            ...
            if self.structures(UnitTypeId.FACTORY).exists and self.already_pending(UnitTypeId.SIEGETANK)+self.units(UnitTypeId.SIEGETANK).amount<=10:
                for fac in self.structures(UnitTypeId.FACTORY):
                    if fac.has_techlab and len(fac.orders) < 2:
                        fac.train(UnitTypeId.SIEGETANK)
    async def train_hellion(self):
        if self.time_formatted>='10:00':
            if self.structures(UnitTypeId.FACTORY).exists:
                for fac in self.structures(UnitTypeId.FACTORY):
                    if not fac.has_techlab and len(fac.orders) < 2:
                        fac.train(UnitTypeId.HELLION)
    async def train_thor(self):
        if self.time_formatted <= '07:00':
            if self.structures(UnitTypeId.FACTORY).exists and self.units(UnitTypeId.THOR).amount <= 1:
                for fac in self.structures(UnitTypeId.FACTORY):
                    if fac.has_techlab and len(fac.orders) < 1:
                        fac.train(UnitTypeId.THOR)
        elif '07:00' <= self.time_formatted <= '09:00':
            if self.structures(UnitTypeId.FACTORY).exists and self.units(UnitTypeId.THOR).amount + self.already_pending(
                    UnitTypeId.THOR) <= 3:
                for fac in self.structures(UnitTypeId.FACTORY):
                    if fac.has_techlab and len(fac.orders) < 1:
                        fac.train(UnitTypeId.THOR)
        elif '11:00' <= self.time_formatted:
            if self.structures(UnitTypeId.FACTORY).exists and self.units(UnitTypeId.THOR).amount + self.already_pending(
                    UnitTypeId.THOR) <= 10:
                for fac in self.structures(UnitTypeId.FACTORY):
                    if fac.has_techlab and len(fac.orders) < 1:
                        fac.train(UnitTypeId.THOR)
    async def thor_mode_change(self):
        if self.units(UnitTypeId.THOR).amount>0:
    ...
    async def train_medic(self):
        if self.structures(UnitTypeId.STARPORT).exists:
            for star in self.structures(UnitTypeId.STARPORT):
                if self.units(UnitTypeId.MEDIVAC).amount < 4 and star.has_reactor and len(star.orders) < 2:
                    star.train(UnitTypeId.MEDIVAC)
    async def train_viking(self):
        if self.structures(UnitTypeId.STARPORT).exists:
            for star in self.structures(UnitTypeId.STARPORT):
                if self.units(UnitTypeId.VIKING).amount < 10 and star.has_reactor and len(star.orders) < 2:
                    star.train(UnitTypeId.VIKING)
    ...
    async def attack(self):
        ...
        if self.supply_army > 100 and self.rally_defend == False:
            ...
            for unit in self.units.of_type({UnitTypeId.MARINE, UnitTypeId.SIEGETANK, UnitTypeId.MARAUDER,UnitTypeId.HELLION,UnitTypeId.THOR,UnitTypeId.THORAP}):
    ...
    async def defend(self):
        ...
        for OC in self.structures(UnitTypeId.ORBITALCOMMAND) | self.structures(UnitTypeId.COMMANDCENTER):
            if self.enemy_units.amount >= 2 and self.enemy_units.closest_distance_to(OC) < 30:
                self.rally_defend = True
                for unit in self.units.of_type({UnitTypeId.MARINE, UnitTypeId.SIEGETANK, UnitTypeId.MEDIVAC,UnitTypeId.THOR,UnitTypeId.HELLION,UnitTypeId.THORAP,UnitTypeId.MARAUDER}):
        ...
        if self.rally_defend == True:
            ...
            for unit in self.units.of_type({UnitTypeId.MARINE, UnitTypeId.MEDIVAC, UnitTypeId.SIEGETANK,UnitTypeId.THOR,UnitTypeId.HELLION,UnitTypeId.THORAP,UnitTypeId.MARAUDER}):
...
def main():
    run_game(  # run_game is a function that runs the game.
        ...
        [Bot(Race.Terran,tank_heller_thor_Bot()),
         Computer(Race.Protoss, Difficulty.Hard)],  # runs a pre-made computer bot, zerg race, with a hard difficulty.
