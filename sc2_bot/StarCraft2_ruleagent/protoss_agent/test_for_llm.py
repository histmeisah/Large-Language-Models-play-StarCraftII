...
class WarpGateBot(BotAI):
    ...
    def get_information(self):
        ...
        information=generate_summarize_L1(information)
        return information
    ...
    async def general_actions(self):

        if self.auto_mode:
            await self.building_supply()
            if self.building_supply_str == None:
                self.building_supply_str = "building_supply"

            await self.expansion()
            if self.expansion_str == None:
                self.expansion_str = "expansion"

            await self.produce_worker()
            if self.produce_worker_str == None:
                self.produce_worker_str = "produce_worker"

            await self.build_vespene()
            if self.build_vespene_str == None:
                self.build_vespene_str = "build_vespene"

        if self.workers.amount >= 66:
            await self.CHRONOBOOSTENERGYCOST_upgrade()
            if self.CHRONOBOOSTENERGYCOST_upgrade_str == None:
                self.CHRONOBOOSTENERGYCOST_upgrade_str = "CHRONOBOOSTENERGYCOST_upgrade"

        if self.expansion_flag:
            await self.expansion()
            if self.expansion_str == None:
                self.expansion_str = "expansion"

        if self.zealot_train:
            await self.train_zealot()
            if self.train_zealot_str == None:
                self.train_zealot_str = "train_zealot"

        if self.stalker_train:
            await self.train_stalker()
            if self.train_stalker_str == None:
                self.train_stalker_str = "train_stalker"

        if self.ht_train:
            await self.train_ht()
            if self.train_ht_str == None:
                self.train_ht_str = "train_ht"

            await self.train_archon()
            if self.train_archon_str == None:
                self.train_archon_str = "train_archon"

        await self.stalker_blink()
        if self.stalker_blink_str == None:
            self.stalker_blink_str = "stalker_blink"

        await self.research_blink()
        if self.research_blink_str == None:
            self.research_blink_str = "research_blink"

    def fill_transaction_with_actions(self):
        self.transaction['building_supply'] = self.building_supply_str
        self.transaction['expansion'] = self.expansion_str
        self.transaction['produce_worker'] = self.produce_worker_str
        self.transaction['build_vespene'] = self.build_vespene_str
        self.transaction['CHRONOBOOSTENERGYCOST_upgrade'] = self.CHRONOBOOSTENERGYCOST_upgrade_str
        self.transaction['train_zealot'] = self.train_zealot_str
        self.transaction['train_stalker'] = self.train_stalker_str
        self.transaction['train_ht'] = self.train_ht_str
        self.transaction['train_archon'] = self.train_archon_str
        self.transaction['stalker_blink'] = self.stalker_blink_str
        self.transaction['research_blink'] = self.research_blink_str

    async def on_step(self, iteration: int):
        self.iteration = iteration

        # 初始化所有动作记录为空
        self.initialize_action_strings()
        ...
        await self.general_actions()

        with self.lock:
            self.transaction['information'] = information
            self.fill_transaction_with_actions()
            self.transaction['iter'] = iteration
            self.logger.info(str(self.transaction))
...
def main():
    ...
    lock = manager.Lock()
    transaction = manager.dict()
    isReadyForNextStep = multiprocessing.Event()  # 假设这是一个事件，用于通知主进程子进程已经完成了一步
    run_game(
        maps.get(LADDER_MAP_2023[0]),
        [Bot(Race.Protoss, WarpGateBot(transaction, lock,isReadyForNextStep)), Computer(Race.Terran, Difficulty.Medium)],
        realtime=False,
    )
