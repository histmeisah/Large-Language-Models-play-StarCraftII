...
class Gemini_vs_llm_Agent:
    ...
    def __init__(self, model_name, api_key, api_base, system_prompt, example_prompt, temperature,player_id, args,
                 action_description,
        ...
        self.base_save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'log', 'gemini_vs_human')
        self.action_db_manager = self.init_action_db()
        # 为每一局游戏创建一个独立的文件夹，这里使用当前时间作为唯一标识符
        self.game_folder = os.path.join(self.base_save_dir, f"game_{self.current_time}_{self.process_id}_{player_id}")
    ...
    def init_action_db(self):
        ...
        if self.player_id ==1:
            player_race =self.args.player1_race
        elif self.player_id ==2:
            player_race= self.args.player2_race
        else:
            raise ValueError("Only support 2 players")
    ...
    def get_opposite_bot_name(self):
        ...
        elif self.args.opposite_type == 'llm':
            self.opposite_name ="llm"
        else:
    ...
    def generate_game_info(self):
        ...
        game_info = f"Map_{self.args.map_pool[self.args.map_idx]}_Player_race_{self.args.player1_race}_vs_{self.args.player2_race}_opposite_type_{self.args.opposite_type}_{self.player_id}"
        return game_info
