...
import os
from sc2_rl_agent.starcraftenv_test.config.config import map_race
from sc2_rl_agent.starcraftenv_test.env.bot.Protoss_bot import Protoss_Bot
from sc2_rl_agent.starcraftenv_test.env.bot.Zerg_bot import Zerg_Bot
    ...
    def check_process(self, agent_num, reset=False):
        ...
        if reset:
            ...
            p = multiprocessing.Process(target=agent_vs_agent, args=(
                ...
                self.map_name, self.process_id, self.args
            ))
            p.start()
...
def agent_vs_agent(transaction1, transaction2, lock1, lock2,
                   ...
                   map_name, process_id, args):
    map = map_name
    current_dir = os.path.dirname(os.path.abspath(__file__))

    # 根据agent_type决定保存路径
    if args.agent1_type in ["random", "llama2", "glm2", "chatgpt","gpt4","glm3","prompt1"] and args.agent2_type in ["random", "llama2", "glm2", "chatgpt","gpt4","glm3","prompt1"]  :
        base_dir = os.path.join(current_dir, '..', '..', '2agent_log', f'{args.agent1_type}_vs_{args.agent1_type}_log')
    else:
        print(f"Unknown agent1_type: {args.agent1_type}, Unknown agent1_type: {args.agent2_type},Defaulting to random_vs_random.")
        base_dir = os.path.join(current_dir, '..', '..', '2agent_log', 'random_vs_random_log')
    ...
    temp_replay_path = os.path.join(replay_folder,
                                    f'{args.current_time}_{map}_Agent1_{args.agent1_type}_vs_Agent2_{args.agent2_type}_process_{process_id}_temp.SC2Replay')

    # 运行游戏并获取结果
    result = run_game(maps.get(map),
                      [Bot(map_race(args.player1_race), agent1(transaction1, lock1, isReadyForNextStep1)),
    ...
    if os.path.exists(temp_replay_path):
        clean_result = clean_filename(str(result))
        final_replay_path = os.path.join(replay_folder,
                                         f'{args.current_time}_{map}_Agent1_{args.agent1_type}_vs_Agent2_{args.agent2_type}_process_{process_id}_{clean_result}.SC2Replay')
        os.rename(temp_replay_path, final_replay_path)
    else:
        print(f'File not found: {temp_replay_path}')
        return
