import re
import openai
from sc2_rl_agent.starcraftenv_test.LLM.glm2_test import ChatBot_SingleTurn
...
class L2_summary:
    ...
    def __init__(self, LLMapi_base, LLMapi_key, model_name, temperature, system_prompt, example_prompt, chunk_window):
        """
        初始化
        :param LLMapi_base:
        :param LLMapi_key:
    ...
    def query(self, inputs):
        ...
        for chunk in chunks:
            ...
            # 使用模板填充
            formatted_input = self.template.input_template.format(
...
#         self.example_prompt = example_prompt
#         self.chunk_window = chunk_window
#         self.chatbot = ChatBot_SingleTurn(api_base=self.api_base, api_key=self.api_key, model_name=self.model_name, temperature=self.temperature, system_prompt=self.system_prompt, example_prompt=self.example_prompt)
#
#     def split_into_chunks(self, L1_summaries):
#         """
#         将L1_summaries依照self.chunk_window的大小拆分成一些chunk
...
#         for chunk in chunks:
#             chunk_str = "\n".join(f"chunk{i}: {item}" for i, item in enumerate(chunk))
#             L2_summary = self.chatbot.query(chunk_str)
#             L2_summaries.append(L2_summary)
#         return L2_summaries
